...
public class ReactiveSources {
	...
	public static Flux<Integer> intNumbersFluxWithException() {
		return Flux.range(1, 10).delayElements(Duration.ofSeconds(1)).handle((e, sink) -> {
			if (e == 5) {
				sink.error(new RuntimeException("An error happened in the flux"));
				return;
			}
			sink.next(e);
		});
	...
	public static Flux<User> userFlux() {
		return Flux.just(new User(1, "Lionel", "Messi"), new User(2, "Cristiano", "Ronaldo"), new User(2, "Diego", "Maradona"),
						new User(4, "Zinedine", "Zidane"), new User(5, "Jürgen", "Klinsmann"), new User(6, "Gareth", "Bale"))
				.delayElements(Duration.ofSeconds(1));
	}

	public static Mono<User> userMono() {
		return Mono.just(new User(1, "Lionel", "Messi")).delayElement(Duration.ofSeconds(1));
	...
	public static Flux<Integer> fallbackFlux() {
		// a fallback sequence of -1 and -2
